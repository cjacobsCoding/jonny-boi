...
|--------|-----------------|-------------|--------|
| feat/inverse-suggest-and-pickers | integrator (jb-pickers) | **docs**: NEW `docs/plans/inverse-suggest-and-card-pickers.md`. **packages/sim**: NEW `scripts/inonly-probe.mjs` (a measurement tool, no src change yet). PLANNED surface when the lane builds: `apps/web/src/components/lab/SuggestPanel.tsx` (an Bring-in focus mirroring the existing cut focus), `apps/web/src/components/lab/CardPicker.tsx` + `card-picker.css` + `lib/lab/cardPicker.ts` (a dwell preview and a named >10 threshold), `packages/sim/src/cli.ts` (an `--in` flag). ⚠️ **THE FINDING, measured not argued:** Caleb asked for a mode that cuts cards to fit ONE specific card in. `generateCandidates` already takes `inOnly` beside `cutOnly` (`suggest-candidates.ts:114-115`), plumbed through `suggest.ts` -> `suggest-run.ts`, pinned by six cases in `suggest.test.ts`, and used in production by `joint-moves.ts:327`. It is exposed NOWHERE a person can reach: `cli.ts:1159` wires only `cutOnly` and `apps/web` has ZERO occurrences of `inOnly`. Measured on the shipped pool: unrestricted 15,628 candidates over 7,810 distinct IN cards; `inOnly: [Sol Ring]` gives 2 candidates, 1 distinct IN, OUT varying over Grizzly Bears and Lightning Bolt. **So this item is an EXPOSURE, not an engine feature.** ⚠️ **NOT CHECKED, by name:** whether `lib/lab/cardPicker.ts` matching is already fuzzy, and whether any card chooser exists OUTSIDE a `<select>` element — the central question of the picker item, deliberately left to a committed sweep because a search for the wrong name returns exactly what a genuine absence returns. ⚠️ **COLLISION WARNING:** `feat/lab-verdict-tuning` is live in the SAME Lab surface (TrimPanel, the panels that render a verdict, `lab-config.ts`) and is changing the RETURN TYPE of `decideVerdict`, which every A/B surface calls. Land that lane first. | 📋 PLANNED — plan + probe committed and pushed, build not started (held on the RAM cap of 2) |
