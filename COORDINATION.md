- 2026-09-11 worker (fix/strionic-ability-copy): ⚠️ **§3.147 COLLIDED WITH `feat/copy-templates` AND I MOVED TO §3.148 — §3.148+ IS FREE AS OF THIS SCAN.** Re-ran the previous worker's cross-branch check over every remote head: only four branches claim anything at or above §3.140, and **`feat/copy-templates` is the only other §3.147**, so it keeps the number and nobody has to move on my account (the same courtesy `feat/block-selectors-effective-pt` did at §3.146). The re-runnable check, which takes a couple of minutes over this many branches: `for b in $(git ls-remote --heads origin | awk '{print $2}' | sed 's|refs/heads/||'); do echo "$b: $(git show "origin/$b:DESIGN.md" 2>/dev/null | grep -o '^### 3\.1[0-9][0-9]' | sort -u | tr '
' ' ')"; done` — ⚠️ **and a section number is a CROSS-FILE identifier**: mine lived in exactly two files (`DESIGN.md` heading, this board), which is why it was a two-line move rather than an eleven-place one. Grep the number, do not edit the heading alone.
- 2026-09-11 worker (fix/strionic-ability-copy): **REPORT 20260911_194411 ANSWERED, AND THE COPIER WAS NOT THE BUG. DESIGN SECTION TAKEN: §3.148** (above §3.146, per the collision note below). *"used strionic resonator twice to exile enemy creatures and didnt copy properly."* Driven through the real `GameSession` the copy is CORRECT — above the original, inherited aim, the "you may choose new targets" question genuinely asked whenever a second legal creature exists. What was wrong: **a copy that exiled a second creature and one that could only keep the original's aim printed BYTE-IDENTICAL logs**, because `abilityActivated`, `triggerCopied`, `triggerFizzled` and the `zoneChange` that IS an exile all fell through `describeEvent`'s `default: return null`. ⚠️ **THAT `default` IS THE CLASS, NOT THE FOUR EVENTS** — 50 of 85 `GameEvent` types were in it and the four this cost were indistinguishable from the 46 that belong there. Both formatters (`apps/web/src/lib/play/play-format.ts`, `apps/web/src/lib/replay-format.ts`) now classify EVERY type as narrated / silent-by-design / **silent-and-that-is-a-gap** (listed apart, so the debt is countable), closed by a `never` check: **adding a GameEvent type without deciding here now FAILS THE BUILD.** ⚠️ **If you add a `GameEvent` to core, both files need a row** — that is the intended cost. Core also gained **CR 608.2b for triggers** in `resolveTriggeredAbility` (a trigger whose targets are all illegal no longer resolves; it emits the `triggerFizzled` the intervening-"if" check already emitted) — **blast radius 0 across 21,885 tests**, and spells deliberately keep their per-mode no-op, which `resolveSpell` already documents as correct for modal spells. Measured: gauntlet **97/320 = 30.3% before and after**, rows 17·14·19·7·8·10·17·5 identical. **12 sabotages, 0 escaped.** Files: `packages/core/src/engine.ts`, `apps/web/src/lib/play/{play-format,session}.ts`, `apps/web/src/lib/replay-format.ts`, + 2 test files. **No overlap with `feat/copy-templates`** (checked its diff first: it owns `cards/compile/rules.ts`, `copy-primitives.ts`, `core/targeting.ts`, `core/triggers.ts` and `generateLegalActions`/`applyActivateAbility` in engine.ts — I touch none of them).
