- 2026-09-11 worker(fix/honest-test-registry): ✅ **SHIPPED: the AI test fixture that could not fail — a registry that swallowed every primitive but one (§3.143).** `packages/ai/src/test-support.ts`'s `createTestRegistry` registered only `dealDamage` and handed back core's plain registry, whose `get` answers an unknown id with `undefined` → `applyEffectRef` emits `effectUnsupported` → **nothing in a test listens for that event**, so the pilot is driven correctly, the ability resolves, nothing happens, and the assertion passes. ⚠️ **THE ENGINE IS RIGHT TO STAY QUIET AND THE FIXTURE WAS WRONG TO COPY IT** — core must degrade an unimplemented card rather than crash a game; a fixture has no player to protect and the only thing its silence can produce is a suite that has stopped measuring. 📊 **BLAST RADIUS MEASURED BEFORE ANY CHANGE, by instrumenting `get` to record every unregistered lookup with its test name and running the whole `ai` suite: 1,129 silent no-ops, but across only 2 FILES and 4 TESTS on 2 primitives** — `destroyTarget` ×869 (`tactical-suite.ts`) and `loseLife` ×260 (`targeting.test.ts`). The static count is ten times bigger and much less interesting (the fixture mints 5 unregistered primitives; 21 files call the factory) — **the closer measurement shrank the estimate and the smaller number is the one reported**, because most fixtures are only ever SCORED and scoring reads the definition, never the body. ⚠️ **ZERO tests were vacuous in the strict sense** — nothing asserted "the effect happened" and got away with it. What was there is worse in one case: **`runTacticalSuite`'s docstring asserted the exact opposite of what its code did** — *"the registry is threaded in so a look-ahead pilot rolls out at full fidelity — without it `Murder` no-ops inside the search and the removal puzzle would be grading the pilot on a game where removal does nothing"* — and the next line built `createTestRegistry()`. The removal category was graded on precisely that game, 869 times. A false comment over a green test is the check reporting that it checked. 📊 **AND THE HONEST FOLLOW-UP: fixing it changed NO score** — scoreboard identical before and after (heuristic 11/12, both hybrids 12/12, `removal 1/1` for all three), because the puzzle grades the chosen ACTION. The suite was one puzzle away from its own comment mattering; what is fixed is that the claim can no longer quietly stop being true. ✅ **THE FIX IS (a)+(c): REFUSE, DON'T IMITATE.** `get` now THROWS on an unregistered id, naming the primitive and both one-line escapes; `has`/`ids` stay honest. **Option (b) was checked and rejected on evidence, not on the rule alone:** `packages/ai/package.json` declares `@jonny-boi/core` ONLY and `tsconfig.json` EXCLUDES both `test-support.ts` and `tactical-suite.ts` from the build — neither is shipped source and neither is exported from `index.ts`, so the "only its TESTS may reach for `cards`" carve-out *arguably* covers them — **but copying the bodies fixes one ROW and refusing the unknown fixes the SHAPE**, and `destroyTarget` alone reaches five `cards`-private helpers. So the boundary is crossed by the CALLER: `createTestRegistry(bodies?)` layers a supplied registry underneath, and a `*.test.ts` (seven already import `cards`) writes `createTestRegistry(buildRegistry())`. ⚠️ **`runTacticalSuite(pilot, { registry })` — the registry is now a REQUIRED field of an options object**, so that module can no longer build a crippled one for itself. ⚠️ **THE NEIGHBOURING TRAP, COUNTED AND FIXED: `applyAction(state, action, { registry })` in NINE places** — 3 in `packages/ai/src/copy-target-pilot.test.ts`, 6 in `packages/core/src/copy.test.ts`. The registry lands in the config slot, the real parameter stays `undefined`, every primitive degrades to `effectUnsupported`, test stays green. All nine fixed, and **`applyAction` now REFUSES a third argument with no `startingLife`** and names the correct call — the mistake is unspellable at runtime for every present and future caller. Behaviour-neutral in fact (the six core sites passed an EMPTY registry, so nothing that mattered was dropped — the shape was the defect). **Not on `applyActionInPlace`:** MCTS rollout path, ~20k calls/decision, no clone to hide behind, both callers internal. Its cost was measured and **reported inconclusive**: interleaved A/B of two builds in ONE process (`cpuUsage`, 8×20k) said +0.66 µs/call, and the CONTROL with the order swapped flipped the sign to −0.39 — the delta follows the SLOT, not the guard, so no cost is claimed. ⚠️ **DEFERRED WITH ITS COUNT, AND IT IS THE ROOT CAUSE OF BOTH BUGS: `{ registry }` is a plain TYPE ERROR, and NO tsconfig in this repo compiles a `*.test.ts`.** Every package excludes them. Compiling all of them today reports **180 errors — 83 `cards`, 41 `core`, 31 `ai`, 18 `sim`, 3 `apps/server`, 2 `protocol`, 2 `apps/web`** — and the `ai` sample is the same family of silent lie (`"Island"` where the type is `"island"` in a LANDWALK test, `step: "main1"` which is not a `Step`, `expect(result.rejected)` on an `EngineResult` with no such field, two fixtures declaring a `targets` key `CardDefinition` does not have). **Blocker: it is a repo-wide cleanup that would collide with every branch in flight and needs its own brief** — not half-done here. **Sabotages: 18 run, 18 caught, 0 escaped**, via committed tooling `scripts/sabotage-honest-test-fixtures.mjs`, which requires RED **from the test that names it** (a row caught by a different test has not shown that test works) and counts a missing anchor as an ESCAPE rather than a pass. ⚠️ **TWO ESCAPED ON THE FIRST DRAFT** — the drift guard drove `dealDamage` through `applyAction`, so the copy's `amount <= 0` and fizzled-target returns were reached by no case and breaking either stayed green; **a sabotage that stays green is a claim about the sabotage before it is a claim about the test** (§3.28's lesson, re-learned). Rewritten through core's `applyEffectRef` it reaches every branch, including the three whose whole contract is to do nothing. 📊 **Gauntlet `"Mono-Red Aggro" --games 40 --seed 99`: 97/320 = 30.3%, byte-identical** — confirmed, not assumed, because a vacuous test that starts really running can reveal a live bug. ⚠️ **FOR THE INTEGRATOR — SEQUENCING, because three siblings are writing NEW `ai` tests against this fixture right now (`feat/copy-templates`, `feat/block-selectors-effective-pt`, `feat/phyrexian-hybrid-mana`).** This branch does not break a test that uses `createTestRegistry()` for `dealDamage` or for pure SCORING — which is almost all of them. It DOES turn red, loudly and with an actionable message, any new test that RESOLVES a primitive the fixture does not register; the fix is one word, `createTestRegistry(buildRegistry())`, and the thrown error says so. **Merge this branch AFTER those three if they are close, so their authors read the error rather than a conflict.** Two files outside `packages/ai` are touched and both are narrow: `packages/core/src/engine.ts` (one call + one new private function, adjacent to `applyAction`) and `packages/core/src/copy.test.ts` (six mechanical call-site fixes) — `feat/copy-templates` should expect a small conflict in the latter and **keep both sides**: its content plus `undefined, registry` in the argument list.
