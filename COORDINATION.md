- 2026-09-11 supervisor: 🚧 **CLAIMED: §3.143 — the MTGA-parity play surface.** Scope is
  **[docs/MTGA-UX-OVERHAUL.md](docs/MTGA-UX-OVERHAUL.md)** (§5 has the full lane→files table; §6 has
  the build-gate rules for this box). Nine disjoint lanes, run as one dynamic workflow from the
  primary checkout: **F** foundations (`play-config.ts`, `keyword-glossary.ts`) → **E** provenance in
  core → then **A** stack / **B** commit-transaction / **C** prompts / **P** provenance-UI / **G**
  combat arcs / **H** damage animation in parallel → **D** board + wiring LAST.
  ⚠️ **`PlayBoard.tsx` belongs to lane D alone**; every other lane delivers a component with a props
  contract and does not wire it. ⚠️ **`play-config.ts` belongs to lane F alone**, which adds every
  lane's constants up front. ⚠️ **Lane E is the only lane that may touch `packages/core`**, its
  attribution is opt-in/lazy because it sits in the sim hot path, and it reports games/sec before and
  after. ⚠️ **No agent runs the full suite** while siblings work — own test files only
  (`--minWorkers=1 --maxWorkers=2`); the integrator gates once at the end.
  ℹ️ The primary checkout was **83 commits behind `origin/main`** when this started (its staged pile
  was already-merged wave-2 work); it was snapshotted to `backup/pre-sync-20260911` and reset to
  `origin/main` @ 9b4524a before any work began.
