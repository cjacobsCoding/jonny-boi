- 2026-09-02 worker (claude/competent-ishizaka-6e5e7d): 🐛 **FIXED: `planManaPayment` could not plan a HYBRID cost from an empty pool.** Its ranking distance never read `cost.hybrid`, so `{1}{G/W}{G/W}` looked like one generic pip, every colour scored the same, and a B/G dual whose modes are listed B-first (Jungle Hollow, Golgari Guildgate) was tapped for B until nothing was left — Kitchen Finks was "unpayable" from FOUR untapped green sources while Eternal Witness planned fine. Hybrid cards were therefore only ever cast by accident, after mana tapped toward something else happened to be the right colours, which biased every A/B with one in it. The distance heuristic now assigns colours to hybrid symbols EXHAUSTIVELY (mirroring `canPayWithHybrids` — greedy misses `{G/W}{G/U}` against G+W), allocation-free via bitmasks in the scratch buffers, and the public `distanceToPayable` is the same dense function, so the two cannot drift. The non-hybrid hot loop is kept as its own monomorphic function. 📊 Interleaved A/B on `pilot-bench` (1,500 games, seed 7, 2 rounds × 2 runs): main 142/145/157/158, new 148/154/154/152 games/sec — parity, identical outcomes (628/1500). Guards: `mana-plan.test.ts` pins the reported board + exhaustive-assignment cases; `heuristic.test.ts` pins the pilot casting Finks from that board in three taps. core 1,069 passed, ai 464 passed, build green.
...
| feat/poison-family | DESKTOP-90PJPM4 (worker, wave 1) | infect / wither / toxic / poison counters — packages/core (player poison + SBA + damage-as-counters replacement), packages/cards compile rows, packages/ai valuation | 🚧 CLAIMED |
| feat/upkeep-cost-family | DESKTOP-90PJPM4 (worker, wave 1) | echo / cumulative upkeep / suspend / vanishing / fading + "sacrifice unless you pay" upkeep template — packages/core (time/age counters, upkeep pay-or-sacrifice choice, suspend exile-with-counters + free cast), packages/cards compile rows, packages/ai choice | 🚧 CLAIMED |
| feat/combat-keyword-family | DESKTOP-90PJPM4 (worker, wave 1) | exalted / rampage / flanking / landwalk / shadow / myriad / split second / provoke + the combat template gaps ("attacks each combat if able", "can block only creatures with flying", "can't be blocked by more than one", "can block an additional creature", attack/block self-pump triggers) — packages/core combat + KeywordFlags, packages/cards compile rows, packages/ai canBlockByEvasion mirror | 🚧 CLAIMED |
| feat/pilot-100x | DESKTOP-90PJPM4 (worker, wave 1) | packages/ai ONLY (+ bench scripts under packages/sim/bench) — heuristic strength (scoreAttackSet defence model) and decision-function speed, every change behind a HeuristicFeatures flag and judged by feature-ab.mjs two-seed protocol + pilot-decide-bench.mjs | 🚧 CLAIMED |
