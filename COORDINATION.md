...
|--------|-----------------|-------------|--------|
| feat/inverse-suggest-and-pickers | worker (jb-pickers, `C:/Users/Caleb/AppData/Local/Temp/claude/jb-pickers`) | **Items 5 + 6 of `docs/plans/inverse-suggest-and-card-pickers.md` — DESIGN §3.181.** **OWNED:** `apps/web/src/components/lab/SuggestPanel.tsx`, `components/lab/CardPicker.tsx` + `card-picker.css`, `components/lab/suggest-focus.css`, `lib/lab/cardPicker.ts` (+test), NEW `lib/lab/cardOptions.ts`, NEW `lib/lab/suggestFocusSummary.ts` (+test), NEW `components/lab/no-raw-card-dropdown.test.ts`, NEW `scripts/check-card-choosers.mjs` + `.d.mts`, NEW `apps/web/scripts/verify-card-picker-preview.mjs`, NEW `packages/sim/src/cli-flags.test.ts`, DESIGN §3.181. **ADDITIVE:** `packages/sim/src/cli.ts` (the flag table + an entry-point guard), `apps/web/src/lib/sim-protocol.ts` / `lib/sim/run.ts` / `lib/sim/shard-protocol.ts` / `lib/sim/execute.ts` (ONE `inOnly` line each, mirroring `cutOnly`), `apps/web/src/lib/scan/match.ts` (one private function EXPORTED + its score floor made a parameter, default unchanged), `apps/web/src/components/CardHover.tsx` (the preview panel EXPORTED, two props made optional — no behaviour change), `lib/about/changelog.ts` (ONE row). ⛔ Did NOT touch `packages/core`, `packages/cards`, the pool or any generated artifact, the Play board, `lib/lab-config.ts`, or ANY of the multicut lane's files (`packages/sim/src/trim.ts`, `TrimPanel.tsx`, `lib/lab/trimSession.ts`, `RunStatus.tsx`, `views/LabView.tsx`). ⚠️ **THE FINDING, measured not argued, and it made item 5 small:** the engine has taken `inOnly` beside `cutOnly` since the search was written — six cases in `suggest.test.ts` pin it and `joint-moves.ts` uses it in production — and it was reachable from NOWHERE. Re-measured on this branch with `packages/sim/scripts/inonly-probe.mjs`: unrestricted **15,628 candidates over 7,810 distinct IN cards**; `inOnly: ['Sol Ring']` gives **2 candidates, 1 distinct IN**, OUT varying over Grizzly Bears and Lightning Bolt. **Item 5 was an EXPOSURE, not an engine feature**, and the commits say so. 🔴 **SEMANTIC CONFLICTS FOR THE INTEGRATOR.** (1) `Flags.in` in `packages/sim/src/cli.ts` changed from `string | undefined` to `readonly string[]` — `--in` is repeatable now because `suggest` takes a list, and `swap` calls `requireOneCard` and REFUSES two values instead of silently testing the last. Any lane reading `flags.in` as a string fails to type-check; that is the intended signal. (2) `cli.ts` now runs `main()` only when it is the process entry point. **This fixed a live false green:** importing `cli.ts` from a test ran the whole CLI and called `process.exit(0)`, which tore down the Vitest worker — three of four requested test files never ran and the summary still said "1 passed". (3) `CardPicker`'s `value`/`onChange` are now OPTIONAL and a `multiple` prop was added; `SwapPanel` compiles untouched. (4) `CardPickerListing` gained a required `fuzzy` field (a return type, so only producers care). (5) `scan/match.ts`'s `matchOneQuery` is exported and takes an optional `minScore`; every existing scan call keeps its old value byte-identical. ⚠️ **NOT DONE, deliberately:** `views/LabView.tsx` still holds its own private `heroOutOptions` / `poolInOptions`. The shared funnel is the new `lib/lab/cardOptions.ts` and the Suggest panel reads it; pointing LabView at it is a ~3-line follow-up left alone because that file belongs to the multicut lane this wave. Two derivations exist meanwhile, identical by construction. ⚠️ **THE GATE'S KNOWN LIMIT, stated not implied:** `scripts/check-card-choosers.mjs` keys every detector off a pool card identifier, so a chooser over card NAMES is invisible to it. The deck builder's Add dialog (Scryfall names over the network) and the scan fixer (the ~30k Magic name list) are recorded as `out-of-scope` allowlist entries WITH reasons rather than pretended to be covered; widening to a bare `.name` was measured and rejected because it matches decks, pilots and opponents. ⚠️ **Scratchpad collision, for whoever else is on this box:** the multicut lane and this one share `…/5aabfb49-…/scratchpad`, and a commit-message file written there was overwritten mid-session. Use distinct filenames. | 🚧 PUSHED, not merged. 🔎 **GATE, each run separately and UNPIPED:** root `npx vitest run` **exit 0 — "Test Files 540 passed | 1 skipped (541)" / "Tests 30386 passed | 5 skipped (30391)"**, no `Worker exited`, **541 collected == 541 `*.test.{ts,tsx}` on disk**; baseline 538/30,358, so +3 files and +33 tests. `npm run build` **exit 0**. `npm run lint` **exit 0 — "14 problems (0 errors, 14 warnings)"**, the documented baseline, **delta 0** (an intermediate state had 23 problems: 8 warnings in CardPicker.tsx and 1 error in the probe, all removed rather than suppressed). `npx tsc --noEmit -p packages/sim/tsconfig.tests.json` **exit 2, 20 errors — the documented baseline, ZERO in this lane's own test file**. `node scripts/check-card-choosers.mjs` **exit 0**. ⚠️ ONE full-suite run earlier reported a single failure — a **5s TIMEOUT** in `lib/play/keyword-glossary.test.ts`, a file this lane never touched, while a peer lane held the box at ~180 MB free; it passes in the clean run above. Reported rather than hidden. 🧪 **RED-THEN-GREEN, all unpiped.** Units: three sabotages at once (CLI drops `inOnly`; `offersFuzzy` uses `>=`; the summary loses its plain-English branch) → **3 files / 6 tests RED, exit 1**; restored with `git checkout` → **33 passed, exit 0**, and each sabotage broke exactly its own check. Gate: the deleted checkbox grid restored → exit 1; a raw `<select>` of cards → exit 1; the same with every suppression comment invented for it → still exit 1; the same markup only in comments → exit 0 (no false alarm); a stale `exempt` entry → exit 1. Browser: the panel stops sending `inOnly` **and** the dwell set to 0 → **13/15, exit 1** (the IN column became 23 different cards, and "leaving early shows nothing" failed); restored → **15/15, exit 0**. 🖥️ **DRIVEN FOR REAL** (`node apps/web/scripts/verify-card-picker-preview.mjs`, **15/15, exit 0**): the served bundle was confirmed to be THIS worktree by 4/4 source markers in the shipped JS before anything was believed; pausing raised "Arbor Elf" in a 340x474 panel; arrowing down with NO pointer raised "Avacyn, Angel of Hope"; leaving early raised nothing; screenshot `apps/web/verify-out/card-picker/dwell-preview-open.png` 387,221 bytes. Pinning Sol Ring in the Bring-in focus produced a **26-row ranked table whose IN set is exactly ["Sol Ring"] over 26 different OUT cards**. FRESHNESS: newest source 00:49:21, dist bundle 00:51:45 — the build is newer. ⌨️ **CLI, on a bundled deck:** `suggest "Selesnya Blink" --in "Sol Ring"` exit 0, every row IN = Sol Ring, **"Evaluated 4 of 14 candidates"**; the same command WITHOUT `--in` reads **"4 of 109276"** with the IN side varying — that is the whole feature in two numbers. `swap --out X --in A --in B` exits 1 with the honest arity error. |
| feat/lab-verdict-tuning | worker (jb-labtune, `C:/Users/Caleb/AppData/Local/Temp/claude/jb-labtune`) | **Items 3 + 2 of `docs/plans/lab-tuning-plan.md` — the trim ladder and the verdict reason.** **OWNED (do not touch while this is in flight): `packages/sim/src/trim.ts` + `src/trim.test.ts`; `decideVerdict` and the verdict block in `packages/sim/src/swap.ts` + its tests; `StatsConfig` in `packages/sim/src/config.ts`; `apps/web/src/components/lab/TrimPanel.tsx` + `trim-panel.css` + `trim-panel.test.ts`; `apps/web/src/lib/lab/trimSession.ts`; DESIGN §3.179.** ADDITIVE ONLY (rows/fields, no restructuring): `apps/web/src/lib/lab-config.ts`, `apps/web/src/lib/sim/merge.ts`, `packages/sim/src/{manabase-reliability,pilot-ab,suggest-report}.ts`, `apps/web/src/components/lab/{JointPanel,ManabasePanel}.tsx` and the Suggest panel — they render the verdict and must render the REASON. ⛔ **Does NOT touch** `packages/core`, `packages/cards`, the pool or any generated artifact, the Play board, or `packages/sim/src/joint*.ts`'s search logic. 🔴 **SEMANTIC CONFLICT FOR THE INTEGRATOR — THE ONE THAT MATTERS: `decideVerdict`'s RETURN TYPE CHANGES.** It is the single funnel every A/B surface in the repo reads (Suggest, trim, the manabase sweep, the joint search, the pilot A/B, and the web merge), and it stops returning a bare `SwapVerdict` and starts returning a verdict **plus a closed-vocabulary reason**, so the function that decides is the function that explains and no surface re-derives it. **Any lane that adds a `decideVerdict` call site while this is in flight will MERGE CLEANLY AND FAIL TO COMPILE** — the textual diffs do not overlap, the types do. That exact shape has produced 269 compile errors on this repo before, so: **build after merging this, do not trust a clean merge.** `SwapEvaluation` gains a reason field (additive) and `TrimStopReason` splits `'exhausted'` into `no-improvement-conclusive` / `budget-exhausted` / `paused` — any lane that switches on `'exhausted'` is the intended signal, not a merge artifact. 📐 **§3.178 is taken by `feat/infinite-combos` (in review)**, which is why this lane's DESIGN section is **§3.179**. Item 1 of the same plan (multi-card cuts, k > 1) is deliberately NOT built here and is a later lane. Item 4 (the Lab's pinned progress bar) is a separate commit on this same branch and touches `views/LabView.tsx` + every Lab panel — flagged here because that is a wide surface.  🔎 **GATE, each run separately and UNPIPED.** Root `npx vitest run` **exit 0 — "Test Files 531 passed | 1 skipped (532)" / "Tests 30300 passed | 5 skipped (30305)"**, no `Worker exited`, 0 FAIL lines, and **532 collected == 532 `*.test.{ts,tsx}` on disk**. `npm run build` **exit 0, 0 TS errors**. `npm run lint` **exit 0, 14 problems (0 errors, 14 warnings)** — all `react-hooks/*`, the same 14 the joint lane measured, so **delta 0**. ⚠️ The BASELINE was NOT independently re-measured: node_modules did not exist in this worktree until an hour into the session (the gate held while another lane ran a suite), and a second full run would have cost another 40 min on a box that spent the night at 300-800 MB free. Derived instead: 532 files = the brief's 529 + my 3 new test files. 🧪 **ELEVEN sabotages, each watched RED then restored GREEN** (harness committed under the plan's scratch, results in the worker report): the reason collapse, games-to-settle returning 0 instead of ABSENT, the estimate taken from the delta instead of the discordant pairs, `z` pinned while alpha moves, the deepening lever pinned, every round forced unsure, the SURVIVOR rule removed, the budget boundary removed, a stop reason losing its wording, a render site dropping the reason, and the budget checked after the widen. 🖥️ **DRIVEN FOR REAL** on a 63-card deck, 11 workers, Lookahead: **8 rounds**, deepening **60 → 120 → 240 → 480** games/candidate where it used to stop at two rounds, ending *"Out of budget, still unsure"*; every row read a reason beside its verdict. ⚠️ **THAT RUN FOUND A BUG**: the Lab overran a 40,000-game budget to **72,162** because `stepAfterRound` tested the budget only on the DEEPEN branch while `trimDeck` tests it at the top of every round. Fixed, with `trim-session-parity.test.ts` as the guard. ⚠️ **ALSO ADDED**: `packages/sim/tsconfig.tests.json` (copied from `fix/type-check-the-tests`, since no package tsconfig type-checks its own tests) — `npx tsc --noEmit -p` it gives **20 errors, all pre-existing, ZERO in my three new test files**. A `web-labtune` entry was appended to the MAIN repo's `.claude/launch.json` (that is where preview_start reads from) pinned to this worktree with `--prefix`; **without it preview_start serves the PRIMARY tree, which cost me a false green until I checked the served LabView.tsx and found three tabs instead of six.** | 🚧 PUSHED, not merged |
| feat/joint-manabase-search | worker (jb-joint) | **packages/sim**: NEW `src/joint.ts`, `src/joint-moves.ts`, `src/joint-config.ts`, `src/joint.test.ts`, NEW `scripts/joint-throughput.mjs`; `src/manabase.ts` (**ONE signature widened**: `applyManabase`'s parameter is now `Pick<ManabaseVariant,'steps'|'label'>` so there is ONE fold from a step list to a deck, not two); `src/index.ts` (one export block). **apps/web**: NEW `components/lab/JointPanel.tsx` + `joint-panel.css` + `joint-panel.test.ts`; `lib/sim-protocol.ts` (`JointPhaseRequest` + one result variant), `lib/sim/shard-protocol.ts` (NEW `VariantSliceSpec`, `JointPlanJob`/`JointPlanResult` + two union members), `lib/sim/execute.ts` (`runJointPlan` + one case), `lib/sim/plan.ts` (`ManabaseArmSlice.variant` retyped to `VariantSliceSpec`), `lib/sim/run.ts` (`runJointPhaseRun` + one case + the request union widened), `lib/lab/manabaseApply.ts` (**parameter widened to `ApplicableSteps`** — the ONE apply funnel), `lib/useLabSelection.ts` (`LabTabId` gains `'joint'` — ONE WORD, the same line lanes T and M each took), `lib/lab-config.ts` (six `JOINT_*` blocks), `views/LabView.tsx` (tab row, `onApplyJointMove`, one mount), `lib/about/changelog.ts` (one row), DESIGN §3.177, this row. ⚠️ **THE POINT OF THE LANE:** §3.175's land-COUNT sweep trades against an arbitrary partner (`mostPlayedBasic` out, the cheapest nonland with room in), so "23 lands" is really "23 lands AND one more copy of my cheapest spell" and the paired verdict measures two changes as one. **Confirmed in source and MEASURED, not argued:** `JOINT_PARTNER_RULES` keeps the shipped rule as a ROW, and one manabase phase on a deck planted at 30 lands whose true optimum is 24 picks **32 lands under the old rule** (away) and **28 under the new one** (toward) — same base, same slots, both verdicts `better`. ⚠️ **SEMANTIC NOTES FOR THE INTEGRATOR, none textual:** (1) `applyManabase` and `applyManabaseToDeck` both took a whole `ManabaseVariant` and now take the steps+label; every existing caller still compiles, but a lane that NARROWS either back will break the joint search silently — `joint.test.ts` catches it. (2) `ManabaseVariantSliceShardJob.variant` is now `VariantSliceSpec`, a `Pick` of `ManabaseVariant`; a manabase variant is still assignable, and the shard now reads only what it uses. (3) `LabTabId` gains `'joint'` on the same one-word line lanes T and M each edited — a three-way conflict on that union is expected and is a union, so keep all members. (4) The joint phase REUSES `manabase-base-slot-shard` and `manabase-variant-slice-shard`; a change to either now has two consumers. 🔎 **GATE, derived from the diff (`apps/web`, `packages/sim`) plus the CONSUMERS of what changed, each run separately and unpiped:** `npm run build` **exit 0** (whole workspace, 0 TS errors). `vitest packages/sim` **exit 0 — 41 files (40 passed, 1 skipped) / 471 tests (466 passed, 5 skipped)**, no `Worker exited`, 41 collected == 41 `*.test.ts` on disk; the skip is `soak-deep.test.ts`, env-gated by design. `vitest apps/web` **exit 0 — 175 files / 2,366 tests passed, 0 failed**, no `Worker exited`, 175 collected == 175 on disk (main is 174; +1 file is this lane's panel test). Root `npx vitest run` **exit 0 — 529 files (528 passed, 1 skipped) / 30,231 tests (30,226 passed, 5 skipped)**, no `Worker exited`; the measured `origin/main` baseline is 526/1 and 30,195/5, so +2 files and +31 tests — this lane's 30, plus ONE new case in the changelog guard, which iterates the ✅ DESIGN sections and therefore gained one when §3.177 was marked done. `npm run lint` **exit 0, 0 errors, 14 warnings** (baseline 12): the two new ones are `react-hooks/set-state-in-effect` in `JointPanel.tsx`, the SAME rule `TrimPanel.tsx` already trips twice for the identical reason (a finished round arrives as a prop and has to be folded into session state). ⚠️ **FIVE falsifications, each watched RED then restored green** — and TWO of them first exposed checks that COULD NOT FAIL and were fixed before being trusted: sabotaging the land-count rollup to return the first partner it was handed stayed GREEN (inside a finished phase the rows already arrive ranked), so `rollUpLandCounts` is now exported and tested on SHUFFLED rows and ranks by proved verdict before delta; and deleting half the partner-shortlist overflow report stayed green because only one direction was covered. The five: default partner rule → 3 red; rollup ordering → 1; local-optimum stop → 1; budget guard → 2; overflow report → 1. 🏎️ **Throughput** (`node packages/sim/scripts/joint-throughput.mjs 8 3`, committed so it is reproducible): a joint phase over the SAME family §3.175 builds — identical decks, identical seeds — is **1.048× the manabase sweep's games/sec** (55.5 vs 53.0, median of three), i.e. no per-game cost, because it is the same watched runner. The measured-partner family reads 0.687× and that is the DECKS (those variants bring real spells in). Planning **1,065 ms** (manabase phase) / **2,046 ms** (spell phase) against **2,094 ms** for a Suggest plan over the same pool — the spell phase's plan costs what a Suggest plan costs because it IS one, once per phase against thousands of games. 🖥️ **Driven in a real browser, not only static-rendered:** Lab → Joint search on Thune's Life, 11 workers, **863 games at 19 games/s**; the LAND COUNTS table rendered 19/20/21(as built)/22/23 lands each with **PARTNERS TRIED = 4** and its best partner named, every verdict INCONCLUSIVE at that depth (honest — it refused to promote the best of the noise), the budget line read "863 of 12,000 games · 53.3 of 900 seconds · 1 phase over round 1 of 6", and the panel advanced to the spell phase. ⚠️ **NOT PUSHED — the brief forbade `git push`.** The branch `feat/joint-manabase-search` exists locally in `D:/Cool Stuff/Claude/jb-joint` only; the integrator pushes or cherry-picks it. | 🚧 COMMITTED, not pushed |
...
| feat/infinite-combos | worker (jb-combos, Lane C stage 1) | **Infinite combos, stage 1 (§3.178): the engine finds the loop a human is stepping through and offers to run it N times.** **packages/core**: NEW `combo.ts` (+`combo.test.ts` 18, `combo-engine.test.ts` 12) — the structure/resources signature, `findComboLoop`, the closed `COMBO_RESOURCE_KINDS` table (fuel rule + phrasing), `COMBO_REPEAT_CAP` 1000 / `COMBO_REPEAT_DEFAULT` 100 / `COMBO_MAX_CYCLE_ACTIONS` 30; `config.ts` (`RulesConfig.comboDetectionSeats: PlayerId[]`, `[]` by default — a seat LIST, not the brief's boolean, because the action completing a human's loop is usually the pilot's pass); `actions.ts` (`repeatCombo`, `dismissCombo`); `state.ts` (`comboHistory`, `comboWindow`, `comboDismissed`); `engine.ts` (the detection tail of `applyActionToDraft`, the window guard in `dispatchAction`, `applyRepeatCombo`/`applyDismissCombo`, the `generateLegalActions` branch, the per-turn reset in `beginTurn`); `internal/clone.ts` (three fields); `events.ts` (+3: `comboWindowOpened`/`comboRepeated`/`comboDismissed`) and their rows in the FIVE total tables — `internal/triggers-runtime.ts`, `instance-ids.ts`, **`packages/sim/src/observation.ts`**, **`packages/sim/src/soak-config.ts`** (sim is off-limits; the two rows are compile-time-exhaustive tables that refuse to build without them), `apps/web/src/lib/play/proposal.ts` (+ one row in `proposal.test.ts`'s reviewed blocking list); the two new ACTION kinds likewise forced a row in three total switches — **`packages/sim/src/soak.ts`** (`actionIdentity`, an implicitly-exhaustive `switch` tsc refuses without them), **`packages/ai/src/search-stats.ts`** and **`packages/ai/src/mcts.ts`** (`never` defaults) — nothing else in sim or ai touched; `conformance/rules-manifest.ts` (`ACTION_RULES` +2 → CR 732.4; section 732 `not-applicable` → `cited` with the 732.2b/732.5 shortfall); `index.ts` exports. **apps/web**: NEW `components/play/ComboPrompt.tsx` + `combo-prompt.css` + `combo-prompt.test.ts` (8 — markup AND the real `PlayBoard` mounted off a state with a window), NEW `lib/play/combo-view.ts` + `combo-rules.ts` + `combo-view.test.ts` (4); `lib/play/session.ts` (a `rules: RulesConfig` on the session, `comboWindow` getter, `repeatCombo`/`dismissCombo`, `hasMeaningfulChoice` stops on a window); `components/play/PlayBoard.tsx` (the mount, the `∞ loop` highlight through `PermInteraction`, the wait text, quiet while open); `components/play/AbilityPrompts.tsx` (`PromptFace` exported — one prompt-card funnel); `components/play/board-scene.test.ts` (one adoption row); `lib/play/play-format.ts` (three log lines); **outside the brief's list, each a one-line wiring**: `views/PlayView.tsx` (2 × `fromCreated(..., playRulesFor(ai?.seat))`), `lib/play/persist.ts` (the rebuild uses the same rules, or a recorded `repeatCombo` replays into a game with no window). Changelog row `3.177` (`kind: 'app'`). ⚠️ **NOT done**: the ONLINE board — `MaskedGameView` does not carry the window (protocol version + `Room` config + mount); the opponent's CR 732.2b interrupt; "Repeat forever" (stage 2). ⚠️ **Semantic notes for the integrator**: `RulesConfig` gained a required field (every literal in the repo spreads `DEFAULT_RULES`, so nothing else changes); `GameSession.fromCreated` gained an optional 4th arg; the five event tables are mapped types — a lane adding an event type will collide on the same lines, resolve by keeping both rows. | 🚧 PUSHED, not merged |
