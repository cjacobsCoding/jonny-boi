...
| feat/copy-effects | worker | packages/core (NEW copy.ts + copy.test.ts; card/state/choices/events/engine/derived/index, internal clone+zones), packages/cards (compile rules +1 rule & 4 hints & parser block, compile/compile.ts, compile/types.ts, NEW compile/copy-effects.test.ts, fidelity.test.ts 1 line), packages/ai (choices/weights/index + NEW copy-target-pilot.test.ts), packages/sim (observation +1, paired-arms-config + paired-arms + its test), apps/web (about/mechanics +2 witnesses, play/choice-view +1 branch, lib/cards.ts +1 branch), DESIGN §3.21, COORDINATION | 🚧 PUSHED, not merged |
...
- 2026-08-20 worker: `feat/copy-effects` 🚧 PUSHED — **the engine has copy effects now, and they are
  ...
  **3685 passed / 0 failed**; `npm run verify` exit 0; `npm run build` exit 0.
  ...
  4. **The question is asked BEFORE the permanent enters, on BOTH entry paths.** The copied card is
     what decides `entersTapped`, summoning sickness, starting loyalty and starting defense, so a
     question asked after the entry (the shockland's shape) would be too late. `resolveTopOfStack`
     puts the stack object back untouched while the question stands; `applyPlayLand` splits its tail
     into a new `completeLandPlay` so the land play finishes after the answer.
