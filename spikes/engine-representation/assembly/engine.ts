 ...
 *   - every state access is `unchecked()`, so no bounds checks are emitted — the
...
@inline function R(addr: i32): i32 {
  return unchecked(arena[addr]);
}

@inline function W(addr: i32, val: i32): void {
  if (journalEnabled) {
    unchecked((journalAddr[journalTop] = addr));
    unchecked((journalPrev[journalTop] = unchecked(arena[addr])));
    journalTop++;
  }
  unchecked((arena[addr] = val));
}
...
function rewind(m: i32): void {
  while (journalTop > m) {
    journalTop--;
    unchecked((arena[unchecked(journalAddr[journalTop])] = unchecked(journalPrev[journalTop])));
  }
}
...
@inline function cf(defBase: i32, field: i32): i32 {
  return unchecked(CARD_TABLE[defBase + field]);
}
...
function deckCard(p: i32, i: i32): i32 {
  return p == 0 ? unchecked(DECK0[i]) : unchecked(DECK1[i]);
}

export function createGame(seed: i32): void {
  arena.fill(0);
  journalTop = 0;
  unchecked((arena[S_TURN] = 1));
  unchecked((arena[S_STEP] = STEP_MAIN1));
  unchecked((arena[S_WINNER] = -1));
  unchecked((arena[S_RNG] = seed));
  for (let p = 0; p < 2; p++) unchecked((arena[pbase(p) + P_LIFE] = STARTING_LIFE));
  ...
  for (let p = 0; p < 2; p++) {
    ...
    for (let i = 0; i < DECK_SIZE; i++) {
      ...
      unchecked((arena[I_DEF + id] = deckCard(p, i) * CARD_STRIDE));
      unchecked((arena[I_OWNER + id] = p));
      unchecked((arena[I_CONTROLLER + id] = p));
      unchecked((arena[I_ZONE + id] = ZONE_LIBRARY));
      unchecked((arena[lib + i] = id));
    }
    unchecked((arena[pb + P_LIB_COUNT] = DECK_SIZE));
    for (let i = DECK_SIZE - 1; i > 0; i--) {
      unchecked((arena[S_RNG] = rngNext(unchecked(arena[S_RNG]))));
      const j = <i32>((<u32>unchecked(arena[S_RNG])) % (<u32>(i + 1)));
      const t = unchecked(arena[lib + i]);
      unchecked((arena[lib + i] = unchecked(arena[lib + j])));
      unchecked((arena[lib + j] = t));
    }
...
function canPayScratch(defBase: i32): bool {
  let spare = unchecked(potentialPool[POOL_COLORLESS]);
  for (let c = 0; c < NUM_COLORS; c++) {
    const need = cf(defBase, CARD_COST + 1 + c);
    const have = unchecked(potentialPool[c]);
    if (have < need) return false;
    spare += have - need;
  }
  return spare >= cf(defBase, CARD_COST + COST_GENERIC);
...
export function generateLegalActions(): i32 {
  ...
  unchecked((actionBuf[n++] = packAction(ACT_PASS, 0, TARGET_NOTHING, 0)));
  ...
  for (let b = 0; b < bfCount; b++) {
    ...
    unchecked((actionBuf[n++] = packAction(ACT_TAP_FOR_MANA, id, TARGET_NOTHING, 0)));
  }
  ...
  if (sorcerySpeed && R(pb + P_LANDS_PLAYED) < MAX_LANDS_PER_TURN) {
    for (let h = 0; h < handCount; h++) {
      ...
      if (cf(cardOf(id), CARD_KIND) == KIND_LAND) {
        unchecked((actionBuf[n++] = packAction(ACT_PLAY_LAND, id, TARGET_NOTHING, 0)));
      }
    }
  }
  ...
  for (let h = 0; h < handCount; h++) {
    ...
    if (restriction == TARGET_NONE) {
      unchecked((actionBuf[n++] = packAction(ACT_CAST_SPELL, id, TARGET_NOTHING, 0)));
      continue;
    }
    for (let b = 0; b < bfCount; b++) {
      const t = R(BF_BASE + b);
      if (cf(cardOf(t), CARD_KIND) != KIND_CREATURE) continue;
      unchecked((actionBuf[n++] = packAction(ACT_CAST_SPELL, id, t, 0)));
    }
    if (restriction == TARGET_ANY) {
      unchecked((actionBuf[n++] = packAction(ACT_CAST_SPELL, id, TARGET_PLAYER_0, 0)));
      unchecked((actionBuf[n++] = packAction(ACT_CAST_SPELL, id, TARGET_PLAYER_0 + 1, 0)));
    }
  }
  ...
  if (
    ...
    unchecked((actionBuf[n++] = packAction(ACT_DECLARE_ATTACKERS, 0, TARGET_NOTHING, 0)));
    unchecked((actionBuf[n++] = packAction(ACT_DECLARE_ATTACKERS, 0, TARGET_NOTHING, ATTACK_PROFITABLE)));
  }
  ...
  if (
    ...
    unchecked((actionBuf[n++] = packAction(ACT_DECLARE_BLOCKERS, 0, TARGET_NOTHING, 0)));
    if (R(S_ATTACKER_COUNT) > 0) {
      unchecked((actionBuf[n++] = packAction(ACT_DECLARE_BLOCKERS, 0, TARGET_NOTHING, BLOCK_GREEDY)));
    }
  }
...
function buildBlocks(me: i32): i32 {
  ...
  for (let a = 0; a < atkCount; a++) {
    ...
    for (let b = 0; b < bfCount; b++) {
      ...
      if (unchecked(usedMark[id]) == usedGen) continue;
    ...
    if (chosen != -1) {
      unchecked((usedMark[chosen] = usedGen));
      W(BLK_LIST + n, chosen);
      W(BLK_OF + n, attacker);
      n++;
    }
...
function wantsToTap(me: i32): bool {
  ...
  for (let i = 0; i < POOL_SLOTS; i++) unchecked((potentialPool[i] = R(poolBase + i)));
  const bfCount = R(S_BF_COUNT);
  for (let b = 0; b < bfCount; b++) {
    const id = R(BF_BASE + b);
    if (R(I_CONTROLLER + id) != me || R(I_TAPPED + id) != 0) continue;
    ...
    unchecked((potentialPool[color] = unchecked(potentialPool[color]) + 1));
  }
...
function chooseAction(count: i32): i32 {
  ...
  for (let i = 0; i < count; i++) {
    const s = scoreAction(unchecked(actionBuf[i]));
    if (s > bestScore) {
      bestScore = s;
      best = i;
    }
...
export function playGame(seed: i32): i32 {
  ...
  while (R(S_GAME_OVER) == 0 && actions < ACTION_CAP) {
    ...
    if (count == 0) break;
    generated += count;
    const a = unchecked(actionBuf[chooseAction(count)]);
...
export function midGameState(seed: i32): void {
  ...
  for (let i = 0; i < MIDGAME_ACTIONS && R(S_GAME_OVER) == 0; i++) {
    ...
    const count = generateLegalActions();
    if (count == 0) break;
    applyAction(unchecked(actionBuf[chooseAction(count)]));
  }
  journalTop = 0;
}
...
export function searchWorkload(seed: i32, rollouts: i32, depth: i32): i32 {
  ...
  for (let r = 0; r < rollouts; r++) {
    ...
    for (let d = 0; d < depth; d++) {
      ...
      if (count == 0) break;
      rng = rngNext(rng);
      applyAction(unchecked(actionBuf[<i32>((<u32>rng) % (<u32>count))]));
      transitions++;
    }
...
export function benchTransition(seed: i32, iters: i32): i32 {
  midGameState(seed);
  const count = generateLegalActions();
  const action = unchecked(actionBuf[count > 1 ? 1 : 0]);
  let sum = 0;
