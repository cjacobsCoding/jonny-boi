/**
 * ARM B+ — a STEELMAN of flat TypeScript.
 *
 * Arm B lost to the object graph on forward simulation, which invites the reply
 * "your flat arm is just badly written". This arm is the answer to that: it is arm
 * B with the single biggest remaining indirection removed, and it is reported so
 * the language conclusion rests on the BEST flat JavaScript measured, not the first.
 *
 * The change: arm B reads a card field as `cards[arena[I_DEF + id] + field]` — a
 * memory load to find the card row, then a load to read the field. Here each
 * instance gets its OWN copy of its card row in a side table at a power-of-two
 * stride, so `cardOf(id)` is `id << 4` — pure arithmetic, no load at all — and a
 * card-field read is one load instead of two. Card characteristics never change in
 * the model (no copy effects), so the duplication cannot go stale, and the table is
 * derived state rather than game state: it lives outside the arena and outside the
 * undo journal.
 *
 * Costs 120 x 16 x 4 = 7.5 KiB of extra memory. Equivalence with arms A, B and C is
 * re-checked, so this is a pure speedup or it is not reported.
 */
...
/** Power-of-two stride so the row offset is a shift rather than a multiply. */
const B2_STRIDE = 16;
const B2_SHIFT = 4;
/** Per-instance copy of the card row — removes the `arena[I_DEF + id]` load. */
const instCards = new Int32Array(MAX_INSTANCES * B2_STRIDE);
...
function cardOf(instanceId) {
  return instanceId << B2_SHIFT;
}
function cf(defBase, field) {
  return instCards[defBase + field];
}
...
export function createGame(seed) {
  ...
  for (let p = 0; p < 2; p++) {
    ...
    for (let i = 0; i < DECK_SIZE; i++) {
      ...
      const srcRow = deck[i] * CARD_STRIDE;
      const dstRow = id << B2_SHIFT;
      for (let f = 0; f < CARD_STRIDE; f++) instCards[dstRow + f] = cards[srcRow + f];
      arena[I_OWNER + id] = p;
