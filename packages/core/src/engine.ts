...
function resolveTopOfStack(
  ...
  if (!effects || effects.length === 0) {
    finishSpellResolution(state, card, leaveTo, emit);
    checkStateBasedActions(state, emit);
    return;
  }
...
function applyCastSpell(
  ...
  const cost =
    fromZone === 'graveyard' ? grantedFlashback : fromZone === 'exile' ? madnessCost : card.def.cost;
  if (cost) {
...
function patchSpellOnStack(
  ...
  patch: Partial<Pick<SpellStackObject, 'xValue' | 'kicked' | 'boughtBack' | 'awaitingCastChoice'>>,
): void {
...
function askNextCastChoice(state: GameState, spellInstanceId: InstanceId, emit: (e: GameEvent) => void): void {
  ...
  // BUYBACK is the same shape as the kicker — an optional additional cost asked
  // once, charged by the engine as the answer is accepted — and differs only in
  // what the answer means later: not a branch inside the spell's script, but
  // where the card goes as it resolves (`spellLeaveDestination`).
  const afterKicker = spellOnStack(state, spellInstanceId);
  if (!afterKicker) return;
  if (def.buyback && afterKicker.boughtBack === undefined) {
