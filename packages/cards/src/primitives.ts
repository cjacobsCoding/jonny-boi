...
export const drawCards: EffectPrimitive = (ctx) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // stop rather than fabricate a loss event here. Checked BEFORE the draw so
    // the behaviour is exactly what it was before this call went through core.
    if (player.library.length === 0) return;
    // Core's draw, not a second copy of it: "if you would draw a card, draw two
