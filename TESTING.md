...
| `loop-runaway.test.ts` | **A game that could not END, and the ruling it needs.** Drives a real soak game into a runaway and requires the soak to REPORT it; pins a CR 104.4b MANDATORY loop and a pilot-that-will-not-stop side by side, and asserts the two read differently (DESIGN §3.139) |
...
by the door nobody was looking at (§3.139); no stack object survives a turn; state-based actions leave no 0-toughness
